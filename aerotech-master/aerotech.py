...
class Ensemble:
    ...
    def enable(self):
        "Enable the axis"
        #self.write_read('ENABLE A')
        self.write_read('ENABLE B')
    ...
    def reset(self):
        """This method homes the stage."""
        self.write_read('ACKNOWLEDGEALL')
        logging.info('Homed')
    ...
    def move(self, x_pos, y_pos, z_pos):
        ...
        """
        command = "MOVEABS X%f XF2.0 Y%f YF2.0 Z%f ZF2.0" % (
            x_pos, y_pos, z_pos)
        #command = "MOVEABS Y -10 YF 2.74"
    ...
    def get_positions(self):
        ...
        eth_status = float(self.write_read('GETMODE(6)'))
        positions = {'X':x_pos, 'Y':y_pos, 'Z':z_pos, 'Eth':eth_status}
