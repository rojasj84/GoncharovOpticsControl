...
class ThreeAxisControlPanel(tk.Frame):
    ...
    def __init__(self, container, x_location, y_location, axis_names, axis_direction_correction, motor_comm_object):
        ...
        jog_up.bind("<ButtonPress>", lambda event:  self.motor_inc_motion(axis_names[1],"+",axis_direction_correction[1]))
        ...
        jog_down.bind("<ButtonPress>", lambda event:  self.motor_inc_motion(axis_names[1],"-",axis_direction_correction[1]))
        ...
        jog_right.bind("<ButtonPress>", lambda event:  self.motor_inc_motion(axis_names[0],"+",axis_direction_correction[0]))
        ...
        jog_left.bind("<ButtonPress>", lambda event:  self.motor_inc_motion(axis_names[0],"-",axis_direction_correction[0]))
        ...
        focus_in.bind("<ButtonPress>", lambda event:  self.motor_inc_motion(axis_names[2],"+",axis_direction_correction[2]))
        ...
        focus_out.bind("<ButtonPress>", lambda event:  self.motor_inc_motion(axis_names[2],"-",axis_direction_correction[2]))
    ...
    def motor_inc_motion(self, axis, direction, axis_direction_correction):        
        #Convert the inputs from mm to microns.
        step_size_float = float(self.step_size_text.get())/1000*axis_direction_correction
...
if __name__ == "__main__":
    ...
    B = ThreeAxisControlPanel(window, 5, 5,["nanoX","nanoY","nanoZ"],[1,-1,-1], A)
